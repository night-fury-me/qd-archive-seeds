...
from qdarchive_seeding.core.exceptions import ConfigError, RegistryError
...
def _build_metadata_relevance_filter(name: str, options: dict[str, object]) -> Transform:
    ...
    if not isinstance(embed_thr, (int, float)):
        embed_thr = 0.35
    ref_path = options.get("reference_embeddings_path")
    if not ref_path or not isinstance(ref_path, str):
        msg = (
            "metadata_relevance_filter requires 'reference_embeddings_path' in config options. "
            "Set it to the path of the .npz file generated by _build_reference_embeddings.py "
            "(e.g. 'metadata/reference_embeddings.npz')."
        )
