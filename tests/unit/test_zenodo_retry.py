...
class TestFailedPageRecording:
    ...
    def test_failed_page_recorded_in_checkpoint(self, tmp_path: Path) -> None:
        ...
        records = list(extractor._extract_single_query(ctx, "test query", query_string="q1"))
    ...
    def test_query_not_marked_complete_with_failures(self, tmp_path: Path) -> None:
        ...
        list(extractor._extract_single_query(ctx, "test query", query_string="q1"))
...
class TestDateSliceCache:
    ...
    def test_date_slices_cached_after_computation(self, tmp_path: Path) -> None:
        cp = CheckpointManager(_path=tmp_path, _pipeline_id="test_pipeline")
