...
def test_build_policy_from_registry(minimal_config: PipelineConfig) -> None:
    ...
    policy = _build_policy(minimal_config, registries, fresh_download=False, retry_failed=False)
    assert isinstance(policy, IncrementalPolicy)

    policy = _build_policy(minimal_config, registries, fresh_download=False, retry_failed=True)
